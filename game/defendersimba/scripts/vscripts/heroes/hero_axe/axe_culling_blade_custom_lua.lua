    ...
    if IsServer() then
        ...
        if caster:HasScepter() then -- если есть скипетр
            ...
            for _, enemy in pairs(targets) do
                if enemy:GetHealth() < damage and not (target:GetUnitName() == "npc_dota_roshan" or target:GetUnitName() == "npc_dota_miniboss")  then-- кому нельзя наносить урон
                    enemy:Kill() -- если здоровье целей меньше чем урона то в любом случае убьет все цели даже под ультом Оракла или под крестом дазла. А и медузу тоже)
                    if enemy:IsHero() then caster:IncrementKills(target:GetPlayerID()) end
                else
                    ApplyDamage({
                        victim = enemy,
                        attacker = caster,
                ...
                if not enemy:IsAlive() then --если цели не выжили то добавляем стаки за них
                    local count
                    if enemy:IsHero() then count = str_per_hero else count = str_per_creep end -- если герой то даем стаки как за героя если крип то как за крипа
            ...
            if target:GetHealth() < damage and not (target:GetUnitName() == "npc_dota_roshan" or target:GetUnitName() == "npc_dota_miniboss") then -- кому нельзя наносить урон
                target:Kill() -- если здоровье цели меньше чем урона то в любом случае убьет все цели даже под ультом Оракла или под крестом дазла. А и медузу тоже)
            ...
            if not target:IsAlive() then -- если цель не выжила то добавляем стак
                local count
                if target:IsHero() then count = str_per_hero else count = str_per_creep end -- если герой то даем стаки как за героя если крип то как за крипа
                local mod = caster:FindModifierByName("modifier_axe_culling_blade_custom_lua")
                for _ = 1, count do
                    mod:IncrementStackCount()
                end
